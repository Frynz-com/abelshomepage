          ...
          {stats.map((stat, index) => (
            <motion.div
              ...
              className={`px-6 lg:px-12 py-10 lg:py-14 ${
                index >= 2 ? "border-t border-cream/8 lg:border-t-0" : ""
              }`}
            >
              <span className="block font-serif text-[38px] lg:text-[48px] text-cream leading-none tracking-[-0.025em] mb-3">
                {stat.value}
              </span>
              <span className="block text-[9.5px] uppercase tracking-[0.28em] text-gold mb-1.5 font-medium">
                {stat.label}
              </span>
              <span className="block text-[12px] text-cream/30 tracking-[0.04em]">
                {stat.detail}
              </span>
            </motion.div>
          ))}
